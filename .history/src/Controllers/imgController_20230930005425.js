...
import { decodeToken } from "../Config/jwtConfig";
import fs from 'fs';
...
const getImg = async (req, res) => {
    ...
    if (!userId) {
        ...
        try {
            ...
            if (images.length > 0) {
                res.status(200).send(images, 'Lấy hình ảnh thành công');
            } else {
        ...
        if (methods.length === 1 && methods[0] === "userId") {
            ...
            try {
                ...
                res.status(200).send(images, 'Lấy hình ảnh theo người dùng thành công');
            } catch (error) {
                console.log(error);
                res.status(500).send('Bất ổn gòi BE ');
            }
        } else {
            res.status(400).send('Lỗi truy vấn');
        }
    }
};
const getImgById = async (req, res) => {
    ...
    try {
        ...
        res.status(200).send(image, 'Lấy hình ảnh thành công');
